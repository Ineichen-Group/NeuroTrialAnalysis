...
from src.exploration import find_mask_words
...
def find_cluster_center(tsne, colors, legend, subset = True, subset_size = 500000, rs = 42):
    ...
    for i in range(len(words)):
        ...
        if cluster.shape[0] > 0:  # Check if the cluster is not empty
            # center with kernel density
            kde = gaussian_kde(cluster.T)
            center = cluster[kde(cluster.T).argmax()]
        else:
            # Handle the case where the cluster is empty. 
            # For example, assign NaNs or a specific value to indicate no cluster center.
            center = np.array([np.nan, np.nan])
            print("no cluster for ", words[i])
        center_cluster_coordinates.append(center)
